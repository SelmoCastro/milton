            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {combos.map((combo, index) => (
                        <div
                            key={index}
                            className={`relative bg-slate-800/50 border ${combo.featured ? 'border-amber-500 shadow-[0_0_20px_rgba(234,179,8,0.15)] scale-105 z-10' : 'border-slate-700 hover:border-slate-600'} rounded-2xl p-8 transition-all hover:translate-y-[-5px]`}
                        >
                            {combo.featured && (
                            ...
                            <p className="text-slate-400 text-sm mb-6 h-10">{combo.description}</p>
